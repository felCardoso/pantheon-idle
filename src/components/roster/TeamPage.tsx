...
export function TeamPage({
  ...
  async function confirmRename() {
    if (renamingSlot !== null && renameValue.trim()) {
      await teams.renameTeam(renamingSlot, renameValue.trim());
    }
    setRenamingSlot(null);
  }
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-5">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        ...
        <div className="flex flex-col gap-4">
            ...
            {teams.teams.map((t) => {
              ...
              return (
                <div key={t.slot} className="flex items-center">
                  {renamingSlot === t.slot ? (
                    <div className="flex items-center gap-1 rounded-lg border border-code-400/50 bg-void-800 px-2 py-1.5">
                      <input
                        autoFocus
                        value={renameValue}
                        ...
                        className="w-24 bg-transparent font-mono text-xs text-white focus:outline-none"
                      />
                    </div>
                  ) : locked ? (
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-lg border border-void-600 bg-void-800/40 px-3 py-2 text-xs text-white/40 transition hover:border-signal-amber/40 hover:text-signal-amber disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:border-void-600 disabled:hover:text-white/40"
                    >
          ...
          <div className="flex flex-wrap gap-2 rounded-xl border border-void-600 bg-void-800/40 p-3">
            {Array.from({ length: MAX_TEAM_MEMBERS }, (_, i) => activeTeamRoster[i]).map((c, i) =>
              c ? (
                <button key={c.templateId} onClick={() => removeFromTeam(c.templateId)} className="group relative">
                  <CharacterPortrait name={c.name} element={c.element} faction={c.faction} portraitUrl={c.portraitUrl} size={64} />
                  <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 rounded-full border border-white/20 bg-void-950/90 px-1.5 py-0 font-mono text-[9px] text-white">
                    Nv.{c.level}
                  </span>
                  <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-void-950/70 opacity-0 transition group-hover:opacity-100">
                    <Icon name="x" size={18} className="text-signal-red" />
                  </div>
                </button>
              ) : (
                <div key={`empty-${i}`} className="flex h-16 w-16 items-center justify-center rounded-xl border border-dashed border-void-600 text-white/20">
            ...
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
              {filteredSelector.map((c: RosterCharacter) => {
                ...
                return (
                  <button
                    key={c.templateId}
                    onClick={() => toggleAdd(c.templateId)}
                  ...
                  >
                    ...
                    <span className="absolute left-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full border border-white/25 bg-void-950/80 px-1 font-mono text-[9px] font-bold text-white">
                      {c.level}
                    </span>
                    {inTeam && (
